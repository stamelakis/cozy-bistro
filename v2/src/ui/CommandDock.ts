 ...
 * player reaches for constantly — Build, Recipes, Upgrades, Pantry, Staff,
 * Decor — and SMALL square tiles (icon + tooltip) for everything else.
 * Reuses the Hud's typed action map so both surfaces stay in lockstep;
 * desktop/mobile visibility is swapped by TopStrip's injected CSS.
 */
...
export class CommandDock {
  constructor(host: HTMLElement, actions: HudActions, toggleBuild: () => void) {
    const tiles: Tile[] = [
      // ── WIDE — the constant-use actions ─────────────────────────────
      { icon: "🔨", label: "Build", tooltip: "Open / collapse the build palette (place furniture)", wide: true, click: toggleBuild, tint: "rgba(220, 170, 110, 0.25)" },
      { icon: "🍽", label: "Recipes", tooltip: "Choose which dishes you serve (opens the recipe carousel)", wide: true, click: actions.openRecipes, tint: "rgba(230, 170, 120, 0.22)" },
      ...
      { icon: "💬", label: "Chat", tooltip: "Chat with the street (opens the chat panel)", wide: false, click: actions.openChat, tint: "rgba(150, 200, 230, 0.22)" },
...
function makeTile(t: Tile): HTMLButtonElement {
  ...
  btn.onclick = (e) => { e.preventDefault(); t.click(); };
  return btn;
}
