    ...
    blocks: [
      { kind: "h", text: "Roles" },
      {
        ...
        items: [
          ...
          "Each chef, waiter and barman is assigned to ONE floor — set it with the floor buttons in the Staff panel — and works there. They won't wander to another floor to pick up slack, so if one floor gets slammed while another sits quiet, move staff over yourself. (Errand helpers roam the whole building.)",
          "One exception: if a dish can only be cooked on a station that sits on another floor — say a single grill or pizza oven shared by the whole building — the chef cooks it on that floor and a WAITER carries the plate up or down the stairs to the guest. The game strongly prefers the guest's own floor and only reaches across when that floor genuinely can't make the dish, so give a busy floor its own full kitchen if you'd rather skip the stair trips.",
