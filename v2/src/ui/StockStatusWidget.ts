...
export class StockStatusWidget {
  ...
  update(): void {
    ...
    // Phase 9.30 — per-tier "Sell" buttons removed from this popup;
    // selling lives in the Pantry → Dishware → STOCK rows now, so there
    // are no dish-sell-btn elements left to wire here.
 ...
 * lower-tier stock they've outgrown. Phase 9.30 — the per-tier "Sell"
 * button was removed from this popup; selling now lives only in the
 * Pantry → Dishware → STOCK rows (9.27), per the user's request, so the
 * info tooltip stays a read-out (plus the −1 cleanup). Buttons carry
